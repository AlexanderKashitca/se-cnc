///-----------------------------------------------------------------------------
/// KinematicsGeppetto.cpp: implementation of the KinematicsGeppettoClass class.
///-----------------------------------------------------------------------------
#include "kinematicsGeppetto.h"
///-----------------------------------------------------------------------------
#define sqr(x) ((x)*(x))
///-----------------------------------------------------------------------------
KinematicsGeppettoClass::KinematicsGeppettoClass()
{
...
void KinematicsGeppettoClass::Rotate(double xc,double yc,double zc,double z,double a,double b,
...
void KinematicsGeppettoClass::Rotate3(double xc,double yc,double zc,double x,double y,double z,double a,double b,double c,
								double *xp,double *yp,double *zp)
{
	double ar=a*PI/180.0;
	double br=b*PI/180.0;
...
///-----------------------------------------------------------------------------
int KinematicsGeppettoClass::TransformCADtoActuators(double x, double y, double z0, double a, double b, double c, double *Acts, bool NoGeo)
{

//#define AX_factor 0.0  // degrees per inch
//#define BX_factor -0.3
//#define AY_factor -0.6
//#define BY_factor 0.4

#define ZYP ((-0.6+1.15-2.2-0.87-0.97-1.0-0.90-1.19) / 25.4)  // enter same signed entered
#define ZYM ((-0.2+0.25+0.2+0.64+0.73+0.89+0.89+0.92) / 25.4)
#define DY 2.0 // inches moved in Y

	double Pb = (ZYP - ZYM) / (2.0*DY);  // correct slope/bow along Y
	double Pa = (ZYP - Pb * DY) / (DY*DY);
	double z = z0 + Pa * (y*y) + Pb * y;
...
//	a = a0 + AX_factor*x + AY_factor*y;
//	b = b0 + BX_factor*x + BY_factor*y;
...
///-----------------------------------------------------------------------------
int KinematicsGeppettoClass::InvertTransformCADtoActuators(double *Acts, double *xr, double *yr, double *zr, double *ar, double *br, double *cr, bool NoGeo)
{
	double Tol=1e-6;
...
int KinematicsGeppettoClass::TransformActuatorstoCAD(double *Acts, double *xr, double *yr, double *zr, double *ar, double *br, double *cr, bool NoGeo)
{
...
// .
// .
int KinematicsGeppettoClass::ReadGeoTable(const char *name)
{
	double X, Y, Z, c;
	int row, col;
...
int KinematicsGeppettoClass::GeoCorrect(double x, double y, double z, double a, double b, double c,
...
///-----------------------------------------------------------------------------
int KinematicsGeppettoClass::GetSoftLimits(double *xm, double *xp, double *ym, double *yp, double *zm, double *zp,
