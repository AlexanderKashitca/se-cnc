...
///-----------------------------------------------------------------------------
/**
 * @note Interface functions to call to tell the interpreter what to do.
 *  Return values indicate status of execution.
 *  These functions may change the state of the interpreter.
 */

/**
 * @brief rs274ngc_close
 * @return
 * @note close the currently open NC code file
 */
extern int rs274ngc_close();

/**
 * @brief rs274ngc_execute
 * @return
 * @note execute a line of NC code
 */
 ...
 * @brief rs274ngc_init
 * @return
 * @note get ready to run
 */
extern int rs274ngc_init();

/**
 * @brief rs274ngc_load_tool_table
 * @return
 * @note load a tool table
 */
 ...
 * @param mdi
 * @return
 * @note read the mdi or the next line of the open NC code file
 */
extern int rs274ngc_read(const char * mdi = nullptr);

/**
 * @brief rs274ngc_reset
 * @return
 * @note reset yourself
 */
...
/**
 * @brief rs274ngc_restore_parameters
 * @param filename
 * @return
 * @note restore interpreter variables from a file
 */
extern int rs274ngc_restore_parameters(const char * filename);

/**
 * @brief rs274ngc_save_parameters
 * @param filename
 * @param parameters
 * @return
 * @note save interpreter variables to file
 */
extern int rs274ngc_save_parameters(const char * filename, const double parameters[]);

/**
 * @brief rs274ngc_synch
 * @return
 * @note synchronize your internal model with the external world
 */
extern int rs274ngc_synch();
///-----------------------------------------------------------------------------
/**
 * @note Interface functions to call to get information from the interpreter.
...
/**
 * @brief rs274ngc_active_g_codes
 * @param codes
 * @note copy active G codes into array [0]..[11]
 */
extern void rs274ngc_active_g_codes(int * codes);

/**
 * @brief rs274ngc_active_m_codes
 * @param codes
 * @note copy active M codes into array [0]..[6]
 */
extern void rs274ngc_active_m_codes(int * codes);

/**
 * @brief rs274ngc_active_settings
 * @param settings
 * @note copy active F, S settings into array [0]..[2]
 */
extern void rs274ngc_active_settings(double * settings);

/**
 * @brief rs274ngc_error_text
 * @param error_code
 * @param error_text
 * @param max_size
 * @note  copy the text of the error message whose number is error_code into the
 *         error_text array, but stop at max_size if the text is longer.
 */
extern void rs274ngc_error_text(int error_code, char * error_text,int max_size);

/**
 * @brief rs274ngc_file_name
 * @param file_name
 * @param max_size
 * @note copy the name of the currently open file into the file_name array,
 *        but stop at max_size if the name is longer
 */
extern void rs274ngc_file_name(char * file_name, int max_size);

/**
 * @brief rs274ngc_line_length
 * @return  the length of the most recently read line
 */
extern int rs274ngc_line_length();

/**
 * @brief rs274ngc_line_text
 * @param line_text
 * @param max_size
 * @note  copy the text of the most recently read line into the line_text array,
 *         but stop at max_size if the text is longer
 */
extern void rs274ngc_line_text(char * line_text, int max_size);

/**
 * @brief rs274ngc_sequence_number
 * @return the current sequence number (how many lines read)
 */
extern int rs274ngc_sequence_number();

/**
 * @brief rs274ngc_stack_name
 * @param stack_index
 * @param function_name
 * @param max_size
 * @note  copy the function name from the stack_index'th position of the
