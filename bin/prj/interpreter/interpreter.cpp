    ...
    case M_Action_Program_wait_sync:
        ...
        else
        {
            ...
            else if (p->String[0])
            {
                ...
                {
                    Err.ReleaseBuffer();
                    CoordMotion->KMotionDLL->DoErrMsg("Error Compiling and Loading KMotion Program\r\r" + FileName + "\r\r" + Err);
                    return 1;
                }
            }
...
//	if (m_restart || m_ReadToolFile)
    ...
    {
        s="Error unable to determine starting conditions for this line.\r\r";
        if (!FoundX) s+="X? ";
        if (!FoundY) s+="Y? ";
        if (!FoundZ) s+="Z? ";
        if (!FoundA) s+="A? ";
